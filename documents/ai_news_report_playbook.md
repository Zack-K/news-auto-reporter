...
**収集後、記事タイトルからHTMLタグを除去し、クリーンなテキストとして後続処理に渡します。**
...
**LLMプロンプトのチューニング:**
*   **記事要約:** 初学者が読みやすいように、専門用語を避け、具体例や比喩を交えながら、200文字程度で詳しく要約するように調整。
*   **記事選定:** データサイエンス、データエンジニアリング、データ分析の学習者にとって最も有用で、会話のきっかけになりそうな記事を選定する基準を維持。
*   **会話を促すコメント:** 個別の記事に対するコメント生成は廃止。
...
- 記事の要約（`paragraph`）
- 記事間には `divider` を挿入
- **カバー画像は自動設定されません。必要に応じてNotion上で手動で設定してください。** (追加)
...
- 選定されたニュース記事（タイトル、URL、要約、初学者向けポイント）と、会話を促すコメント（`section`） (記事タイトルはHTMLタグ除去済み)
- Dividerで区切り
