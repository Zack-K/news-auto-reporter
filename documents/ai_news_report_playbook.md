...
*   **Python環境:** `actions/setup-python`アクションでPython 3.11環境をセットアップし、`uv`を使用して依存関係をインストールします。
...
各記事から、タイトル、URL、公開日、概要を抽出します。外国語の記事については、日本語に翻訳し、要約を作成します。自動要約に加え、初学者向けのポイント（3行程度）を生成します。会話を促すコメントは、個別の記事ごとではなく、レポート全体のクロージングコメントとして生成されます。
